...
public class BlockSculkNullstone extends Block implements IGrowable {
	...
	public void registerBlockIcons(IIconRegister registry) {
		...
		this.sideIcon = registry.registerIcon("nova_craft:sculk_nullstone");
	}
	...
	public IIcon getIcon(IBlockAccess world, int x, int y, int z, int side) {
		if (side == 1) {
			...
			return NovaCraftBlocks.nullstone.getBlockTextureFromSide(side);
		} else {
	...
	public IIcon getIcon(int side, int meta) {
		...
		if (side == 0) return NovaCraftBlocks.nullstone.getBlockTextureFromSide(side);
		return sideIcon;
	}
