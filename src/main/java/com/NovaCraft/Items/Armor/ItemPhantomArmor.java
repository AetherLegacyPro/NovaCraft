...
public class ItemPhantomArmor extends ItemBasePhantomArmor implements ISpecialArmor {
	...
	public ItemPhantomArmor(int armorType, ArmorMaterial material, String name, Item repair) {
		super(armorType, material, name, repair);
	}

	@Override
	public ArmorProperties getProperties(EntityLivingBase player, ItemStack armor, DamageSource source, double damage, int slot) {
		return new ArmorProperties(0, this.calculateIncrease(armor) / 100D, Integer.MAX_VALUE); 
	}
	...
	public void onArmorTick(final World world, final EntityPlayer player, final ItemStack itemStack) {
        ...
        if (hasPherithiumHelmet && hasPherithiumChest && hasPherithiumLegs && hasPherithiumBoots && world.provider.dimensionId == 1) {
