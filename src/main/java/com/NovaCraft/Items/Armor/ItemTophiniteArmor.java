...
public class ItemTophiniteArmor extends ItemBaseTophiniteArmor implements ISpecialArmor {
	...
	public ItemTophiniteArmor(int armorType, ArmorMaterial material, String name, Item repair) {
		super(armorType, material, name, repair);
	}

	@Override
	public ArmorProperties getProperties(EntityLivingBase player, ItemStack armor, DamageSource source, double damage, int slot) {
		return new ArmorProperties(0, this.calculateIncrease(armor) / 32D, Integer.MAX_VALUE);
	}
	...
	public void onArmorTick(final World world, final EntityPlayer player, final ItemStack itemStack) {
        ...
        if (helmet != null) {
            hasPherithiumHelmet = (helmet.getItem() == NovaCraftItems.tophinite_helmet);
        }
        if (chest != null) {
            hasPherithiumChest = (chest.getItem() == NovaCraftItems.tophinite_chestplate);
        }
        if (legs != null) {
            hasPherithiumLegs = (legs.getItem() == NovaCraftItems.tophinite_leggings);
        }
        if (boots != null) {
            hasPherithiumBoots = (boots.getItem() == NovaCraftItems.tophinite_boots);
        }
